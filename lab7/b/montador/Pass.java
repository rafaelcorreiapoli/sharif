...
public abstract class Pass {
    ...
    protected int getDecNumber(String arg) throws AssemblerException, NumberFormatException {
        ...
        String resto = arg.substring(1, arg.length());
        switch (base) {
        	case DECIMAL_CODE:
        		return Integer.parseInt(resto);
        		
        	case OCTAL_CODE:
        		return Integer.parseInt(resto, 8);
        	
        	case HEX_CODE:
        		return Integer.parseInt(resto, 16);
        		
        	case BINARY_CODE:
        		return Integer.parseInt(resto, 2);
        		
        	case ASCII_CODE:
                if (!resto.matches(ASCII_CHARS))
                    throw new NumberFormatException("Número inválido");

                char caractere = resto.charAt(0);
                int ascii = (int) caractere * 256;
                caractere = resto.charAt(1);
                ascii += (int) caractere;
                return ascii;
