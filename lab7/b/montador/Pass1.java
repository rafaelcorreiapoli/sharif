...
class Pass1 extends Pass {
    ...
    private boolean defineNewOrigin(String arg) {
    	try {
            int newLocation = getDecNumber(arg);

            if (newLocation < LAST_VAL_ADDR) {
                locationCounter = newLocation;
                return true;
            }
        } catch (AssemblerException | NumberFormatException exception) {
        }
        return false;
    }
    ...
    private boolean defineConstant() {
    	locationCounter += 2;
        return locationCounter < LAST_VAL_ADDR;
    }
    ...
    private boolean reserveBlock(String arg) {
    	try {
            int reserve = 2 * getDecNumber(arg);
            if (reserve > 0 && (locationCounter + reserve) < LAST_VAL_ADDR) {
                locationCounter += reserve;
                return true;
            }
        } catch (AssemblerException | NumberFormatException exception) {
        }
